...
Debug tool. Patches the Gateway socket's `_doIdentify`/`_doResume`/`_doResumeOrIdentify` and shows
a log of each call's `properties`/presence in its own settings screen (auth token is never shown,
only whether one was present). For investigating what Discord actually reports as your
device/super-properties. Not meant to stay enabled long-term.

### Install

> https://witorsell.github.io/placeholdercord-plugins/GatewayDiagnostics/
