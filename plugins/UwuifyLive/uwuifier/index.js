...
// Build a Uwuifier from the current stored settings. Called on every use so toggling
// options applies instantly, with no manual reload or Discord restart needed.
function buildUwuifier(storage) {
	const settings = storage?.['settings']?.['uwuifier'] ?? {};
	const spaces = settings['spaces'] ?? {};
	return new Uwuifier({
		spaces: {
			faces: !spaces['faces'] ? 0 : 0.5,
			actions: !spaces['actions'] ? 0 : 0.075,
			stutters: !spaces['stutters'] ? 0 : 0.1,
		},
...
export function getUwuifier(storage) {
	// Always reflect the latest settings; fall back to defaults if storage is missing.
	return storage ? buildUwuifier(storage) : new Uwuifier();
}

// Kept for backward compatibility; no longer required since getUwuifier reads live settings.
export function reloadUwuifier() { /* no-op */ }