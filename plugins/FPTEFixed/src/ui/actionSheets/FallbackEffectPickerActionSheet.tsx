...
export function FallbackEffectPickerActionSheet({ currentEffectId, effects, onSelect }: FallbackEffectPickerActionSheetProps) {
    ...
    const colors: ItemProps["colors"] = useMemo(() => [
        semanticColors.BACKGROUND_PRIMARY ? resolveSemanticColor(theme, semanticColors.BACKGROUND_PRIMARY) : "#313338",
        semanticColors.BACKGROUND_FLOATING ? resolveSemanticColor(theme, semanticColors.BACKGROUND_FLOATING) : "#2B2D31",
        semanticColors.BUTTON_OUTLINE_BRAND_BORDER_ACTIVE ? resolveSemanticColor(theme, semanticColors.BUTTON_OUTLINE_BRAND_BORDER_ACTIVE) : "#FFFFFF"
    ], [theme]);
                            ...
                            renderItem={({ item }) => (
                                ...
                                >
                                    {item.map(effect => effect
                                        ? (
                                            <Item
                                                label={effect.config?.accessibilityLabel ?? "Profile Effect"}
