...
import { saveDeleted } from "./persist.js";
			...
			patches.push(
				patchBefore("dispatch", FluxDispatcher, (args) => {
					try {
						...
						// Persist the deletion to disk so it survives restarts (first-pass capture only).
						try { saveDeleted(message, event.channelId); } catch (_) { /* never break delete handling */ }
